...
//  Pickup
//
//  Created by Robert Puryear on 6/23/18.
...
class HomeVC: UIViewController, Alertable {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if Auth.auth().currentUser?.uid != nil {
            DataService.instance.userIsDriver(userKey: (Auth.auth().currentUser?.uid)!, handler: { (status) in
                if status == true {
    ...
    // right now this only works for users
...
//                self.actionBtn.setTitle("START TRIP", for: .normal)
        ...
        self.removeOverlaysAndAnnotations(forDrivers: false, forPassengers: true)
        self.centerMapOnUserLocation()
//        self.buttonsForPassenger(areHidden: true)
        self.actionBtn.isUserInteractionEnabled = true
    }
    
    @IBAction func actionBtnWasPressed(_ sender: Any) {
        self.buttonSelector(forAction: actionForButton)
    }
            ...
            DataService.instance.driverIsOnTrip(driverKey: (Auth.auth().currentUser?.uid)!, handler: { (isOnTrip, driverKey, tripKey) in
                if isOnTrip == true {
                    DataService.instance.REF_TRIPS.child(tripKey!).child("destinationCoordinate").observe(.value, with: { (snapshot) in
                        ...
                        destinationMapItem.name = "Destination Pickup Point"
...
//    func buttonsForPassenger(areHidden: Bool) {
//        if areHidden {
//            self.actionBtn.fadeTo(alphaValue: 0.0, withDuration: 0.2)
//            self.cancelBtn.fadeTo(alphaValue: 0.0, withDuration: 0.2)
//            self.centerMapBtn.fadeTo(alphaValue: 0.0, withDuration: 0.2)
...
//            self.requestRideCancelBtn.isHidden = true
//            self.costLbl.isHidden = true
////            self.centerMapBtn.isHidden = true
//        } else {
//            self.actionBtn.fadeTo(alphaValue: 1.0, withDuration: 0.2)
//            self.cancelBtn.fadeTo(alphaValue: 1.0, withDuration: 0.2)
...
//            self.requestRideCancelBtn.isHidden = false
//            self.costLbl.isHidden = false
////            self.centerMapBtn.isHidden = false
//        }
//    }
    ...
    func performSearch() {
        ...
        search.start { (response, error) in
            if error != nil {
                ...
                self.showAlert("No results. Please search again")
            } else {
...
//        self.buttonsForPassenger(areHidden: false)
    }
